...
SBSuggestionTextBoxCustom::SBSuggestionTextBoxCustom()
	: IgnoreUIUpdate(false)
	, SelectedSuggestion(-1)
{ }

void SBSuggestionTextBoxCustom::Construct(const FArguments& InArgs)
{
	...
	ChildSlot
		...
		.Text(InArgs._Text)
		.OnTextChanged(this, &SBSuggestionTextBoxCustom::HandleTextBoxTextChanged)
		.OnTextCommitted(this, &SBSuggestionTextBoxCustom::HandleTextBoxTextCommitted)
		]
	.MenuContent
	(
		SNew(SBorder)
		...
		.OnGenerateRow(this, &SBSuggestionTextBoxCustom::HandleSuggestionListViewGenerateRow)
		.OnSelectionChanged(this, &SBSuggestionTextBoxCustom::HandleSuggestionListViewSelectionChanged)
		]
		]
	)
		];
...
void SBSuggestionTextBoxCustom::SetText(const TAttribute<FText>& InNewText)
{
	IgnoreUIUpdate = true;
...
void SBSuggestionTextBoxCustom::ClearSuggestions()
{
	SelectedSuggestion = -1;
...
void SBSuggestionTextBoxCustom::FocusTextBox()
{
...
FString SBSuggestionTextBoxCustom::GetSelectedSuggestionString() const
{
...
void SBSuggestionTextBoxCustom::MarkActiveSuggestion()
{
	IgnoreUIUpdate = true;
...
void SBSuggestionTextBoxCustom::SetSuggestions(TArray<FString>& SuggestionStrings, bool InHistoryMode)
{
...
void SBSuggestionTextBoxCustom::OnFocusLost(const FFocusEvent& InFocusEvent)
{
}

FReply SBSuggestionTextBoxCustom::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& KeyEvent)
{
...
bool SBSuggestionTextBoxCustom::SupportsKeyboardFocus() const
{
	return true;
}

TSharedRef<ITableRow> SBSuggestionTextBoxCustom::HandleSuggestionListViewGenerateRow(TSharedPtr<FString> Text, const TSharedRef<STableViewBase>& OwnerTable)
{
	...
	return SNew(STableRow<TSharedPtr<FString> >, OwnerTable)
		[
			...
			[
				SNew(STextBlock)
				.HighlightText(this, &SBSuggestionTextBoxCustom::HandleSuggestionListWidgetHighlightText)
...
void SBSuggestionTextBoxCustom::HandleSuggestionListViewSelectionChanged(TSharedPtr<FString> NewValue, ESelectInfo::Type SelectInfo)
{
	if (!IgnoreUIUpdate)
	{
...
FText SBSuggestionTextBoxCustom::HandleSuggestionListWidgetHighlightText() const
{
	return TextBox->GetText();
}

void SBSuggestionTextBoxCustom::HandleTextBoxTextChanged(const FText& InText)
{
	if (!IgnoreUIUpdate)
	{
		const FString& InputTextStr = TextBox->GetText().ToString();
...
void SBSuggestionTextBoxCustom::HandleTextBoxTextCommitted(const FText& InText, ETextCommit::Type CommitInfo)
{
	if (!MenuAnchor->IsOpen())
	{
...
bool SBSuggestionTextBoxCustom::HasOpenPopup()
{
